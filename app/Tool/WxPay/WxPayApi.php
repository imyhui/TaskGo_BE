...
class WxPayApi
{
    ...
    public static function getRsa(WxPayGetRsa $inputObj,$timeOut = 6){
	    $url='https://fraud.mch.weixin.qq.com/risk/getpublickey';
        $inputObj->setMchId(WxPayConfig::MCHID);
        $inputObj->setNonceStr(self::getNonceStr());
        $inputObj->SetSignType('MD5');
        $inputObj->SetSign();
        $xml = $inputObj->ToXml();
        $response = self::postXmlCurl($xml,$url,true,$timeOut);
        $result = WxPayResults::Init($response,true);
        return $result;
    }

	public static function mmPayToBank(WxPayToBank $inputObj,$timeOut = 6){
	    $url='https://api.mch.weixin.qq.com/mmpaysptrans/pay_bank';
        $inputObj->setMchId(WxPayConfig::MCHID);
        $inputObj->setNonceStr(self::getNonceStr());
        $inputObj->setPartnerTradeNo(PayTool::makeOutTradeNo());
        $inputObj->SetSign();
        $xml = $inputObj->ToXml();
        $response = self::postXmlCurl($xml,$url,true,$timeOut);
        $result = WxPayResults::Init($response);
	...
	public static function shorturl($inputObj, $timeOut = 6)
	{
 	 ...
 	 */
	public static function notify($callback, &$msg,$rsa=false)
	{
		//获取通知的数据
		$xml = file_get_contents('php://input');
		//如果返回成功则验证签名
		try {
			$result = WxPayResults::Init($xml,$rsa);
...
//			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			...
			curl_setopt($ch,CURLOPT_CAINFO,WxPayConfig::CAINFO);
		}
		//post提交方式
